...
pub(crate) fn rewrite(_: syn::AttributeArgs, struct_item: syn::ItemStruct) -> pm::TokenStream {
    ...
    sendable_struct_item.fields.iter_mut().for_each(|f| {
        let ty = f.ty.clone();
        f.ty = syn::parse_quote!(<super::#ty as arc_script::arcorn::Convert>::T);
    });
    ...
    quote!(
        ...
        #[derive(Clone, Debug, arc_script::arcorn::derive_more::Deref)]
        pub struct #abstract_id {
            pub concrete: std::rc::Rc<#concrete_id>,
        }

        impl arc_script::arcorn::Convert for #concrete_id {
            type T = #abstract_id;
        ...
        mod #mod_id {
            ...
            impl arc_script::arcorn::Convert for super::#abstract_id {
                type T = #abstract_id;
                fn convert(self) -> Self::T {
                    #abstract_id {
            ...
            impl arc_script::arcorn::Convert for #abstract_id {
