...
pretty! {
    ...
    ast::ExternTypeItemKind => match node {
        ast::ExternTypeItemKind::ExternMethod(item) => write!(w, "{}", item.pretty(fmt)),
    ...
    ast::ExternFun => write!(w, "extern fun {}({}): {}", node.name.pretty(fmt), node.params.pretty(fmt), node.rt.pretty(fmt)),
    ast::ExternMethod => write!(w, "fun {}({}): {}", node.name.pretty(fmt), node.params.pretty(fmt), node.rt.pretty(fmt)),
    ...
    ast::Task => write!(w, "task {name}({params}): {iinterface} -> {ointerface} {body}",
