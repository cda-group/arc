...
impl Context<'_> {
    ...
    pub(crate) fn new_expr_tuple(&mut self, vs: Vec<hir::Var>) -> hir::Expr {
        self.new_expr(hir::ExprKind::Tuple(vs))
    }
    ...
    pub(crate) fn new_variant(&mut self, enum_path: hir::Path, t: hir::Type) -> hir::Variant {
        let variant_name = self.info.names.fresh();
        let variant_path = self.info.paths.intern_child(enum_path, variant_name).into();
        hir::Variant {
            path: variant_path,
            t,
            loc: Loc::Fake,
