...
pretty! {
    ...
    hir::ExprKind => match node {
        ...
        hir::ExprKind::BinOp(v0, op, v1) => write!(
            w,
            "{v0}{op}{v1}",
            v0 = v0.pretty(fmt),
            op = op.pretty(fmt),
        ...
        hir::ExprKind::UnOp(op, v) => match &op.kind {
            hir::UnOpKind::Not => write!(w, "not {}", v.pretty(fmt)),
            hir::UnOpKind::Neg => write!(w, "-{}", v.pretty(fmt)),
            hir::UnOpKind::Err => write!(w, "☇{}", v.pretty(fmt)),
            _ => unreachable!()
        },
