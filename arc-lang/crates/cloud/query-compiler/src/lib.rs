...
pub fn compile(name: &str, source: &str, query_config: Query, cluster: &mut Cluster) -> Graph7 {
    ...
    let graph5 = lowering5::lower(graph4, cluster, query_config);
    // Compile pipelines to source code.
    let graph6 = lowering6::lower(graph5, cluster);
    // Compile source code to binaries.
    let graph7 = lowering7::lower(&name, graph6, cluster);
    graph7
}
