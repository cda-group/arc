...
pub(crate) fn lower(graph: Graph5, cluster: &Cluster) -> Graph6 {
    let (worker_defs, worker_ids): (Vec<_>, Vec<_>) = graph
        .shards
        .iter()
        .group_by(|shard| shard.worker_id)
        .into_iter()
        .map(|(worker_id, shards)| compile_worker(worker_id, shards, &graph, cluster))
        .unzip();
...
pub fn compile_worker<'a>(
    ...
    let (shard_defs, shard_ids, shard_cpus): (Vec<_>, Vec<_>, Vec<_>) =
        multiunzip(shards.map(|s| compile_shard(s, graph, cluster)));
    ...
    for (id, node) in &pipeline.nodes {
        ...
        match node {
            _ if node.is_source() => {
                ...
                let init_expr = new(id, node, shard, graph, cluster);
...
fn new(
    ...
    match node {
        Node2::KafkaSource {
            ...
            let broker = cluster.broker.to_string();
            let key_type = parse_code(key_type);
            let data_type = parse_code(data_type);
            quote!(KafkaSource::<#key_type, #data_type>::new(#broker, #topic))
        }
        Node2::KafkaSink { topic, .. } => {
            let broker = cluster.broker.to_string();
            quote!(KafkaSink::new(#broker, #topic))
        }
...
fn send(node: &Node2, node_id: Ident) -> TokenStream {
    match node {
        ...
        Node2::KafkaSink { input, .. } => {
            let input = new_node_id(input);
            quote!(#node_id.send(&mut #input, key).await)
        }
        Node2::ShuffleSource { .. }
        | Node2::Filter { .. }
        | Node2::Map { .. }
