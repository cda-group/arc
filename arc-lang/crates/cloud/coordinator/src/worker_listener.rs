...
impl Actor {
    ...
    async fn handle(&mut self, stream: TcpStream, addr: SocketAddr) {
        ...
        tokio::spawn(async move {
            ...
            let mut rx: tcp::Rx<CoordinatorAPI> = Framed::new(rx, Json::default());
            let tx = FramedWrite::new(tx, LengthDelimitedCodec::new());
            let tx: tcp::Tx<WorkerAPI> = Framed::new(tx, Json::default());
            if let Some(Ok(CoordinatorAPI::RegisterWorker { arch })) = rx.next().await {
                tracing::info!("Handshake successful");
                server
                    .register_worker(id, addr.ip(), arch, WorkerTx::start(tx))
                    .await;
                WorkerRx::start(rx, id, server);
            }
        });
    }
}
