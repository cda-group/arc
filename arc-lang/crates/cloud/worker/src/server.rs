...
use crate::runtime_rx::RuntimeRx;
use crate::runtime_tx::RuntimeTx;
use crate::Args;

struct Actor {
    mailbox: mpsc::Receiver<Message>,
    tx: Option<CoordinatorTx>,
    txs: HashMap<u64, RuntimeTx>,
    server: Server,
    args: Args,
    id: u64,
}
    ...
    fn handle(&mut self, msg: Message) {
        match msg {
            ...
            Message::Execute { name, binary } => {
                ...
                self.txs.insert(self.id, RuntimeTx::start(tx));
                RuntimeRx::start(rx, self.id, self.server.clone());
            }
