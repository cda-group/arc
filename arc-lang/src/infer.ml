...
and instantiate_enum enum_xs enum_t ts0 (ctx:Ctx.t) =
  ...
  let ctx = ctx |> unify_ts ts0 ts1 in
...
and mgu t0 t1 ctx : ((Hir.name * Hir.ty) list * Ctx.t) =
  ...
  | Hir.TVar x, t | t, Hir.TVar x ->
      if t = Hir.TVar x then
        ...
        ([(x, t)], ctx)
  | _ ->
      let pr_ctx = Pretty.Ctx.brief in
      Printf.printf "Oops... ";
      Pretty_hir.pr_type t0 pr_ctx;
      Printf.printf " != ";
...
and infer_item (ctx:Ctx.t) (xs, i) : Ctx.t =
  if ctx |> Ctx.has_scheme xs then
    ...
    | Hir.IVal (_, t0, b) ->
        ...
        todo ()
    | Hir.IDef (a, explicit_gs, ps, t0, b) ->
        ...
        let (ps, t0, b) = tmap_def (apply s) (ps, t0, b) in
        ...
        let (ps, t0, b) = tmap_def (generalise s) (ps, t0, b) in

        (* Store the type scheme / generic item *)
        let sc = Ctx.SPoly { t; explicit_gs; implicit_gs } in
        let ctx = ctx |> Ctx.add_item xs (Hir.IDef (a, explicit_gs @ implicit_gs, ps, t0, b)) in
        let ctx = ctx |> Ctx.add_scheme xs sc in

        ctx
    | Hir.ITask (d, explicit_gs, ps0, ps1, b) ->
        let ctx = ctx |> Ctx.push_frame in

        (* Infer MGU of the function *)
        let ctx = ctx |> bind_params (ps0 @ ps1) in
        ...
        (* Apply MGU to all types in the function *)
        let (ps0, ps1, b) = tmap_task (apply s) (ps0, ps1, b) in

        (* Create the task type *)
        let t = match ps1 with
        | [(_, t)] ->
            Hir.TFunc (ts_of_ps ps0, t)
        | _ ->
            let t = ps1 |> ts_of_ps |> Hir.indexes_to_rows Hir.TRowEmpty in
            Hir.TFunc (ts_of_ps ps0, Hir.TRecord t)
        in
        ...
        (* Replace all type variables with generics *)
        let t = t |> generalise s in
        let (ps0, ps1, b) = tmap_task (generalise s) (ps0, ps1, b) in

        (* Store the type scheme / generic item *)
        let sc = Ctx.SPoly { t; explicit_gs; implicit_gs } in
        let ctx = ctx |> Ctx.add_item xs (Hir.ITask (d, explicit_gs, ps0, ps1, b)) in
        let ctx = ctx |> Ctx.add_scheme xs sc in
        ctx
    | Hir.IExternDef (_, explicit_gs, ts, t) ->
        let t = Hir.TFunc (ts, t) in
        let ctx = ctx |> Ctx.add_scheme xs (Ctx.SPoly { t; explicit_gs; implicit_gs = [] }) in
...
and infer_ssa_rhs ctx (v0, t0, e0) =
  ...
  | Hir.ELit l ->
      infer_lit t0 l ctx
  | Hir.ELoop b ->
      let ctx = ctx |> Ctx.push_subctx (Ctx.CLoop t0) in
      let (t1, ctx) = infer_block b ctx in
      ctx |> unify t0 (atom "unit")
          |> unify t1 (atom "unit")
          |> Ctx.pop_subctx
