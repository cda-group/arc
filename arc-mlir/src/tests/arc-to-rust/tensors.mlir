// RUN: arc-mlir -rustcratename arctorusttensors -arc-to-rust -crate %t -extra-rust-trailer %s.rust-tests %s && CARGO_HTTP_DEBUG=true cargo test -j 1 --manifest-path=%t/arctorusttensors/Cargo.toml
// RUN: arc-mlir -rustcratename arctorusttensorscanon -canonicalize -arc-to-rust -crate %t -extra-rust-trailer %s.rust-tests %s && CARGO_HTTP_DEBUG=true cargo test -j 1 --manifest-path=%t/arctorusttensorscanon/Cargo.toml
