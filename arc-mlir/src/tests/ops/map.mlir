...
module @toplevel {
  ...
  func.func @flip_map(%in : !arc.stream<ui32, ui32>)
    ...
    %out = "arc.map"(%in) { map_fun=@flip, map_fun_env_thunk=@_flip_env_thunk} : (!arc.stream<ui32, ui32>) -> !arc.stream<ui32, ui32>
  ...
  func.func @flip_map_no_env(%in : !arc.stream<ui32, ui32>)
  	    -> !arc.stream<ui32, ui32> {
    %out = "arc.map"(%in) { map_fun=@flip_no_env} : (!arc.stream<ui32, ui32>) -> !arc.stream<ui32, ui32>
    return %out : !arc.stream<ui32, ui32>
  }
}
