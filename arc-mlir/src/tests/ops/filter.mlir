...
module @toplevel {
  ...
  func.func @even_filter(%in : !arc.stream<ui32, ui32>)
  	    -> !arc.stream<ui32, ui32> {
    %out = "arc.filter"(%in) { predicate=@is_even} : (!arc.stream<ui32, ui32>) -> !arc.stream<ui32, ui32>
    return %out : !arc.stream<ui32, ui32>
  }

  func.func @chained_filter(%in : !arc.stream<ui32, ui32>)
  	    -> !arc.stream<ui32, ui32> {
    %t = "arc.filter"(%in) { predicate=@is_even} : (!arc.stream<ui32, ui32>) -> !arc.stream<ui32, ui32>
    %r = "arc.filter"(%t) { predicate=@is_odd} : (!arc.stream<ui32, ui32>) -> !arc.stream<ui32, ui32>
    return %r : !arc.stream<ui32, ui32>
  }
  ...
  func.func @filter_with_env(%in : !arc.stream<ui32, ui32>)
  	    -> !arc.stream<ui32, ui32> {
    %out = "arc.filter"(%in) { predicate=@is_same_with_env, predicate_env_thunk=@one_thunk } : (!arc.stream<ui32, ui32>) -> !arc.stream<ui32, ui32>
    return %out : !arc.stream<ui32, ui32>
  }
