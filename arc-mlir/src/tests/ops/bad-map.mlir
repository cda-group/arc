...
// expected-note@+1 {{see current operation}}
    %out = "arc.map"(%in) { map_fun=@flip} : (ui32) -> !arc.stream.source<ui32, ui32>
...
module @toplevel {
  ...
  func.func @flip_map(%in : !arc.stream.source<ui32, ui32>)
  	    -> ui32 {
// expected-error@+2 {{'arc.map' op result #0 must be a source stream, but got 'ui32'}}
// expected-note@+1 {{see current operation}}
    %out = "arc.map"(%in) { map_fun=@flip} : (!arc.stream.source<ui32, ui32>) -> ui32
    return %out : ui32
  }
}
// -----
...
module @toplevel {
  ...
  func.func @flip_map(%in : !arc.stream.source<ui32, si32>)
...
// expected-note@+1 {{see current operation}}
    %out = "arc.map"(%in) { map_fun=@flip} : (!arc.stream.source<ui32, si32>) -> !arc.stream.source<ui32, si32>
...
// expected-note@+1 {{see current operation}}
    %out = "arc.map"(%in) { map_fun=@flip} : (!arc.stream.source<ui32, ui32>) -> !arc.stream.source<ui32, si32>
    return %out : !arc.stream.source<ui32, si32>
  }
}
// -----
...
// expected-note@+1 {{see current operation}}
    %out = "arc.map"(%in) { map_fun=@flip} : (!arc.stream.source<ui32, ui32>) -> !arc.stream.source<ui32, ui32>
    return %out : !arc.stream.source<ui32, ui32>
  }
}
// -----
...
// expected-note@+1 {{see current operation}}
    %out = "arc.map"(%in) { map_fun=@flip, map_fun_env_thunk=@flip_env} : (!arc.stream.source<ui32, ui32>) -> !arc.stream.source<ui32, ui32>
    return %out : !arc.stream.source<ui32, ui32>
  }
}
// -----
